...
The Artemis Benchmarking-Tool is a web application that is used to simulate the workload of a large number of students on an Artemis instance. The application is used to evaluate the performance of Artemis and to identify bottlenecks. It is part of the Artemis project, which is developed at the Chair of Applied Software Engineering at the Technical University of Munich.
...
- `cleanup-enabled`: Whether the tool should clean up (i.e., delete created courses and exams) after the simulation.
- `is-local`: Whether the Artemis Server is using the Integrated Code Lifecycle setup.
...
- If you want to simulate against the Artemis Production instance and chose a mode that requires Instructor rights, you can optionally specify the necessary credentials. _They will be stored in the database!_ Please refer to the "Artemis User Management" section for more information. This step is optional and can be done later.
