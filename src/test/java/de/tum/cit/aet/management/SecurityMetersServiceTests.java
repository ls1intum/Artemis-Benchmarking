package de.tum.cit.aet.management;
...
public class SecurityMetersServiceTests {
    ...
    void testCountMethodsShouldBeBoundToCorrectCounters() {
        assertThat(meterRegistry.get(INVALID_TOKENS_METER_NAME).tag("cause", "expired").counter().count()).isZero();
        ...
        assertThat(meterRegistry.get(INVALID_TOKENS_METER_NAME).tag("cause", "unsupported").counter().count()).isZero();
        ...
        assertThat(meterRegistry.get(INVALID_TOKENS_METER_NAME).tag("cause", "invalid-signature").counter().count()).isZero();
        ...
        assertThat(meterRegistry.get(INVALID_TOKENS_METER_NAME).tag("cause", "malformed").counter().count()).isZero();
