...
- `pnpm run services:up`: start local services via Docker (`src/main/docker/services.yml`).

## Coding Style & Naming Conventions

- Indentation follows `.editorconfig`: 4 spaces by default, 2 spaces for TS/JS/JSON/CSS/SCSS/HTML/YAML.
- Client formatting: Prettier (`pnpm run prettier:format`) with single quotes and 140 char print width; linting via `pnpm run lint`.
